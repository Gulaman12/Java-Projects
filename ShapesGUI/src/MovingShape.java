...
public abstract class MovingShape {
	 ...
	 */
	public MovingShape() {
		this(0, 0, 20, 20, 500, 500, Color.blue, 0, 1); // the default properties
	}
	 ...
	 */
	public MovingShape(int x, int y, int w, int h, int mw, int mh, Color c,  int pathType, float pen) {
		p = new Point(x,y);
		marginWidth = mw;
		marginHeight = mh;
		width = w;
	...
	public void setPenWidth(float pen) {
		penWidth = pen;
	}
	...
	public abstract class MovingPath {
		public static final int FALLING = 0, BOUNCE = 1;// The Id of the moving path
	...
	public class BouncingPath extends MovingPath {
		...
		public void move() {
			...
			p.x = p.x + deltaX;
			if (p.x > marginWidth) // if it reaches the bottom of the frame, start again from the top
				p.x = 0;
			}
	}
